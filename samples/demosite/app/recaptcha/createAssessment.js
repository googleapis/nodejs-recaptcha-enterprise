...
async function createAssessment(projectId, recaptchSiteKey, token, recaptchaAction) {
    ...
    if (!response.tokenProperties || !response.tokenProperties.valid) {
        throw new Error(`The Create Assessment call failed because the token was invalid for the following reasons: ${response.tokenProperties.invalidReason}`);
    }
